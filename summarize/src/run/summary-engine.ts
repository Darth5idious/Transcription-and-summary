...
export type SummaryEngineDeps = {
  ...
  llmCalls: Array<{
    provider: "xai" | "openai" | "google" | "anthropic" | "zai" | "nvidia" | "groq" | "cli";
    model: string;
...
export function createSummaryEngine(deps: SummaryEngineDeps) {
  ...
  const envHasKeyFor = (requiredEnv: ModelAttempt["requiredEnv"]) => {
    ...
    if (requiredEnv === "GROQ_API_KEY") {
      return Boolean(deps.apiKeys.groqApiKey);
    }
  ...
  const runSummaryAttempt = async ({
    ...
    const apiKeysForLlm = {
      ...
      groqApiKey: deps.apiKeys.groqApiKey,
    };

    const modelResolution = await resolveModelIdForLlmCall({
      parsedModel,
      apiKeys: { googleApiKey: apiKeysForLlm.googleApiKey, groqApiKey: apiKeysForLlm.groqApiKey },
    ...
    if (!streamingEnabledForCall) {
      ...
      return {
        ...
        summaryAlreadyPrinted: false,
        modelMeta: {
          provider: parsedModelEffective.provider as "xai" | "openai" | "google" | "anthropic" | "zai" | "nvidia" | "groq",

...
export async function generateTextWithModelId({
  ...
  provider: "xai" | "openai" | "google" | "anthropic" | "zai" | "nvidia" | "groq";
  ...
  while (attempt <= maxRetries) {
    ...
    try {
      ...
      if (parsed.provider === "openai" || parsed.provider === "groq") {
        const openaiConfig = parsed.provider === "groq"
          ? {
        ...
        if (parsed.provider === "groq" && !apiKeys.groqApiKey) {
          throw new Error("Missing GROQ_API_KEY for groq/... model");
        }
...
export async function streamTextWithContext({
  ...
  try {
    ...
    if (parsed.provider === "openai" || parsed.provider === "zai" || parsed.provider === "nvidia" || parsed.provider === "groq") {
      const openaiConfig: OpenAiClientConfig = (() => {
        ...
        if (parsed.provider === "groq") {
          const key = apiKeys.groqApiKey;
          if (!key) throw new Error("Missing GROQ_API_KEY for groq/... model");
          return {
            apiKey: key,
            baseURL: "https://api.groq.com/openai/v1",
            useChatCompletions: true,

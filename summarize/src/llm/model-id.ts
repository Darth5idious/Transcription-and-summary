export type LlmProvider = "xai" | "openai" | "google" | "anthropic" | "zai" | "nvidia" | "groq";
...
const PROVIDERS: LlmProvider[] = ["xai", "openai", "google", "anthropic", "zai", "nvidia", "groq"];
...
export function normalizeGatewayStyleModelId(raw: string): string {
  ...
  if (slash === -1) {
    ...
    if (normalized.startsWith("groq-")) return `groq/${normalized}`;

<script lang="ts">
	...
	let accumulatedAudio: Float32Array = new Float32Array(0);
	...
	async function setup() {
		if (isSafari()) throw new Error("Safari is not supported. Please use Chrome, Edge, or Firefox Nightly.");
		if (!navigator.gpu) {
			throw new Error("WebGPU not available. Ensure you are using a compatible browser (Chrome 113+, Firefox 128+) and have GPU drivers installed. For Firefox, enable 'dom.webgpu.enabled' in about:config.");
		}
	...
	async function startRecording() {
		...
		mic = new Microphone((chunk) => {
			...
			// Periodically run the model if we have enough data (e.g., every 3 seconds)
			if (accumulatedAudio.length >= 16000 * 3 && !isGenerating) {
	...
	async function runLiveTranscription() {
		if (!model || isGenerating || accumulatedAudio.length === lastProcessedLength) return;
		isGenerating = true;
		lastProcessedLength = accumulatedAudio.length;
		
		try {
			// We pass the whole accumulated audio to the model
			// The model returns the full transcription so far
			audioData = accumulatedAudio;
			await runModel();
		} finally {
			isGenerating = false;
		}
	...
	function stopRecording() {
		...
		isRecording = false;
		// Final run
		runLiveTranscription().then(() => {
			// Create a blob URL for the recorded audio so the user can play it back
			const wavHeader = createWavHeader(accumulatedAudio.length, 16000);
			const blob = new Blob([wavHeader, accumulatedAudio.buffer], { type: "audio/wav" });
		...
		{:else if isReady}
			...
			{#if !isGenerating && !result}
				...
				<div class="flex flex-col justify-center items-center w-full mt-64 space-y-8">
					<p class="text-xl text-center">Upload an audio file or start recording.</p>

					<div class="flex flex-row items-center space-x-2">
						<p class="text-sm font-medium">Language:</p>
						<select 
							bind:value={configOptions.language}
							class="bg-white border border-gray-300 rounded-lg px-2 py-1 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
						>
					...
					<div class="flex flex-row space-x-4">
						<button 
							on:click={handleToggleRecord}
							class="rounded-3xl p-[2px] {isRecording ? 'bg-red-500 animate-pulse' : 'bg-gradient-to-r from-blue-500 to-green-500'}"
						>
							<div class="bg-white rounded-[22px] px-5 py-1 flex flex-row items-center justify-center space-x-1">
								<p class="font-[500] text-lg">{isRecording ? "Stop Recording" : "Record"}</p>
								<img src="images/{isRecording ? 'pause' : 'play'}.svg" alt="Record icon" height="24" width="24" class={isRecording ? 'invert' : ''} />
							</div>
						</button>
